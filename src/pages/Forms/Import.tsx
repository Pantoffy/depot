...
export default function NhapKho() {
  ...
  const handleSelectSupplier = (supplierId: number | null) => {
    ...
    const supplier = availableSuppliers.find((s: any) => s.id === supplierId);
    if (supplier) {
  ...
  const filteredAvailableSuppliers = availableSuppliers.filter((supplier: any) => {
  ...
  const filteredReceipts = receipts
    .filter((r) => r.soPhieu.toLowerCase().includes(searchTerm.toLowerCase()) || r.tenNCC.toLowerCase().includes(searchTerm.toLowerCase()))
    .sort((a, b) => {
      let comparison = 0;
        ...
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
          ...
          <div className="p-5 lg:p-6 border-b border-gray-200 dark:border-gray-800">
            <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
              ...
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value as "ngayTao" | "tongTien")} className="px-3 py-2.5 pr-8 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500">
                <option value="ngayTao">Sắp xếp theo ngày</option>
                <option value="tongTien">Sắp xếp theo tiền</option>
              </select>
              <button onClick={() => setSortOrder(sortOrder === "asc" ? "desc" : "asc")} className="p-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-all">
                {sortOrder === "asc" ? "↑" : "↓"}
              </button>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {loading ? (
                  ...
                  <tr key={receipt.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4"><span className={`inline-block px-3 py-1.5 rounded-full text-xs font-medium ${receipt.trangThai === "Đã xác nhận" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400" : "bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-400"}`}>{receipt.trangThai}</span></td>
      ...
      <>
        ...
        <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Số Phiếu</label><input type="text" value={formData.soPhieu} onChange={(e) => setFormData({ ...formData, soPhieu: e.target.value })} placeholder="PN001" className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
            ...
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Số Hóa Đơn NCC</label><input type="text" value={formData.soHoaDonNCC} onChange={(e) => setFormData({ ...formData, soHoaDonNCC: e.target.value })} placeholder="HD-001" className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Kho</label><select value={formData.kho} onChange={(e) => setFormData({ ...formData, kho: e.target.value })} className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="">Chọn kho</option><option value="kho-a">Kho A</option><option value="kho-b">Kho B</option><option value="kho-c">Kho C</option></select></div>
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Số Chứng Từ</label><input type="text" value={formData.soChungTu} onChange={(e) => setFormData({ ...formData, soChungTu: e.target.value })} placeholder="CT-001" className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
          </div>
          ...
          <div className="mb-8 p-4 lg:p-6 border border-gray-200 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-gray-800/30">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
              ...
              <div><label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Đơn Vị</label><select value={materialInput.unitId} onChange={(e) => {const unitId = parseInt(e.target.value); const unit = availableUnits.find((u: any) => u.id === unitId); if (unit) {setMaterialInput({ ...materialInput, donVi: unit.name || '', unitId: unitId });}}} className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"><option value="">Chọn đơn vị</option>{availableUnits.map((unit: any) => (<option key={unit.id} value={unit.id}>{unit.name}</option>))}</select></div>
      ...
      <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] p-6"><p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-3">Trạng Thái</p><p className={`text-sm font-medium ${selectedReceipt.trangThai === "Approved" ? "text-emerald-700 dark:text-emerald-400" : "text-orange-700 dark:text-orange-400"}`}>{selectedReceipt.trangThai === "Approved" ? "Đã xác nhận" : "Chờ xác nhận"}</p></div>
